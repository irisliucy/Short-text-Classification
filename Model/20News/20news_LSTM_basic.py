...
def train_Test_Split(data, labels):
    ...
    X_train = pad_sequences(train_X, maxlen=MAX_SEQUENCE_LENGTH)
    X_test = pad_sequences(test_X, maxlen=MAX_SEQUENCE_LENGTH)
...
def create_model():
    ...
    model.add(Embedding(                          # Layer 0, Start
        input_dim=MAX_NB_WORDS + 1 ,                   # Size to dictionary, has to be input + 1
...
def main():
	...
	for indx, val in enumerate(parameters[key]):
	    ...
	    #embedding_index(GLOVE_DIR, FILENAME)
            data, labels, labels_index = load_data(TEXT_DATA_DIR)      #load dataset
