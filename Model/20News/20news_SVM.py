...
parameters = {
"classes" : [2, 3, 5, 7, 9, 11, 13, 15, 17, 19, 20],
...
def main():
	...
	for indx, val in enumerate(parameters[key]):
	    ...
	    last_epoch_training_loss, last_epoch_training_accuracy, eval_loss, eval_accuracy, trainTime = train_and_evaluate_model(model, train_X, train_Y, test_X, test_Y)
	    total_time = time.time()-global_time
	    print ("Total Training Time : ", total_time)
	    add_csv_result(last_epoch_training_accuracy, 0 , eval_accuracy, total_time)
	write_csv_result("20news_LSTM_CV_" + key + ".csv")
