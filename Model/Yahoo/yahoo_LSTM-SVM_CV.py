...
weights_path = "yahoo_LSTM_model_" + str(Classes) +".h5" 
...
def train_and_evaluate_model(model, train_X, train_Y, test_X, test_Y, data):
    ...
    np.savetxt('yahoo_' + str(MAX_SEQUENCE_LENGTH) + ' D_' + str(Classes)+'_train_output.txt', intermediate_output_train, fmt = '%s')

    intermediate_output_test = model.predict(test_X)
    print ("Intermediate testing output shape : ", intermediate_output_test.shape)
    np.savetxt('yahoo_' + str(MAX_SEQUENCE_LENGTH) + ' D_' + str(Classes)+'_test_output.txt', intermediate_output_test, fmt = '%s')
